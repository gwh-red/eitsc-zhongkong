...
public class UdpServerHandler extends SimpleChannelInboundHandler<DatagramPacket>{
	...
	protected void channelRead0(ChannelHandlerContext ctx, DatagramPacket packet) throws Exception {
		...
		if(hexStr != null) {
			...
			if (hexStr.substring(0, 8).equalsIgnoreCase("BB032100")) {
				// 判断map中是否已存在该中控的信息
				...
				if(FastUtils.isEmpty(macAddress)) {
					...
					HeartStatusMapping.setDateMap(macAddress, new Date());	// 状态更新时间
			...
			if (hexStr.substring(0, 8).equalsIgnoreCase("BB022100")) {
				String macAddress = ChannelMappingMac.getMac(packet.sender());
			...
			if (hexStr.substring(0, 12).equalsIgnoreCase("BB021F000006")) {
